normal_CRC <- read.table("~/LOBICO/data/long.random.forest.any/cpm_selected_features.Normal-CRC.txt", 
                  sep = '\t', header = T, row.names = 1)
normal_HCC <- read.table("~/LOBICO/data/long.random.forest.any/cpm_selected_features.Normal-HCC.txt", 
                         sep = '\t', header = T, row.names = 1)
normal_PAAD <- read.table("~/LOBICO/data/long.random.forest.any/cpm_selected_features.Normal-PAAD.txt", 
                         sep = '\t', header = T, row.names = 1)
## Normal-CRC part
...
for(i in 1:a){
  temp <- sum(normal_HCC[i,])/a + sd(normal_HCC[i,])
  for(j in 1:b){
