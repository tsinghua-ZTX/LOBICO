...
% Import data
data = importdata('~/LOBICO/data/short.random.forest.up/Bi-Normal-CRC.txt');
Norm_CRC_data = data.data';
Y = importdata('~/LOBICO/data/short.random.forest.up/Normal-CRC-biclass.txt');

%% Generate W
%Set equal class weights
W = abs(Y-0.5);
W(Y==1) = 2 * W(Y==1)./sum(W(Y==1));
W(Y==0) = 2 * W(Y==0)./sum(W(Y==0));
